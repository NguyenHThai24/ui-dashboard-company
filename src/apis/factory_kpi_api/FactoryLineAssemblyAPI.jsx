...
export const fetchOuputLineData = async (date, floor) => {
  try {
    const response = await req.get(`/getFloorData?date=${date.format('YYYY-MM-DD')}&floor=${floor}`);
...
export const fetchShoeData = async (date, floor, line) => {
  try {
    const response = await req.get(`/getFloorData?date=${date.format('YYYY-MM-DD')}&floor=${floor}&line=${line}`);
    return response.data;
  } catch (error) {
