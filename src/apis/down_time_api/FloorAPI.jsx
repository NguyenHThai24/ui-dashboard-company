...
import { setLoading, setError, setChartData, setChartDataFixed, setChartRepairingTime,setChartMostDowntime,setChartMostBreakdown, setChartMostRepairing,setChartTotalMachine  } from '@/redux/downTimeSlice';
import { setChartReasonMin, setTableMechanicList, setChartTotalReason, setChartRepairingMethod, setTableRepairingStatus } from "../../redux/downTimeSlice";
...
export const fetchChartMostRepairing = (factory, floor, line, section, dates, datee, lang) => async (dispatch) => {
  dispatch(setLoading(true)); // Set loading to true
...
export const fetchChartTotalMachine = (factory, floor, line, section, dates, datee, lang) => async (dispatch) => {
  dispatch(setLoading(true)); // Set loading to true
...
export const fetchChartReasonMin = (factory, floor, line, section, dates, datee, lang) => async (dispatch) => {
  dispatch(setLoading(true)); // Set loading to true
...
// Gọi API biểu đồ MECHANIC LIST
export const fetchTableMechanic = (factory, floor, line, section, dates, datee, lang) => async (dispatch) => {
  dispatch(setLoading(true)); // Set loading to true
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data.data)) {
      ...
      rawData.forEach((item) => {
        ...
        const { mechanic: mech, mechanic_type: type, current_task: task, status: stat, counts: cnt } = item;
      ...
      // Dispatch the formatted data to Redux store
      dispatch(setTableMechanicList({ mechanic, mechanic_type, current_task, status, counts }));
    } else {
      throw new Error("Invalid response format: No data or wrong structure");
    }
...
export const fetchChartTotalReason= (factory, floor, line, section, dates, datee, lang) => async (dispatch) => {
  dispatch(setLoading(true)); // Set loading to true
...
export const fetchChartRepairingMethod= (factory, floor, line, section, dates, datee, lang) => async (dispatch) => {
  dispatch(setLoading(true)); // Set loading to true
...
// Gọi API biểu đồ REPAIRING QUEUE
export const fetchTableRepairingStatus = (factory, floor, line, section, dates, datee, lang) => async (dispatch) => {
  dispatch(setLoading(true)); // Set loading to true
  ...
  try {
    ...
    if (response.data && Array.isArray(response.data.data)) {
      ...
      rawData.forEach((item) => {
        const { Name_en: name, request: req, waiting: wait, repairing: repai, done: don , floors: flo} = item;
        floors.push(flo);
        Name_en.push(name);
      ...
      // Dispatch the formatted data to Redux store
      dispatch(setTableRepairingStatus({ Name_en, request, waiting, repairing, done, floors }));
    } else {
      throw new Error("Invalid response format: No data or wrong structure");
    }
