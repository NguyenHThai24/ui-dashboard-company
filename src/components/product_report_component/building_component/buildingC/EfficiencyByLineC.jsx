...
import { fetchEfficiencyByLine } from "@/apis/product_report_api/buildingAPI/BuildingCAPI";
import { setLoading, setError } from "@/redux/loading/loadingSlice";

const EfficiencyByLineC = ({ selectedDate }) => {
  const dispatch = useDispatch();
  const { chartDataEfficiency, loading, error } = useSelector((state) => ({
    chartDataEfficiency: state.buildingc.chartDataEfficiency,
    loading: state.buildingc.loading,
    error: state.buildingc.error,
  }));
  ...
  const options = {
    ...
    title: {
      text: "BUILDING C: EFFICIENCY BY LINE",
      align: "center",
...
export default EfficiencyByLineC;
