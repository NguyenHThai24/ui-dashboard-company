...
const RepairingMethodChart = ({ floor, line, date }) => {
  const { chartRepairingMethod, loading, error } = useSelector((state) => state.downtime);
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(fetchChartRepairingMethod(
    ...
    datasets: [
      {
        ...
        backgroundColor: [
          "#ff6384", "#36a2eb", "#cc65fe", "#ffce56", "#17a398",
          "#ffa600", "#bc5090", "#58508d", "#003f5c",
        ],
  ...
  return (
    <Card sx={{ width: "100%", margin: "auto", height: "100%", boxShadow: 10, borderRadius: 2, bgcolor: "white" }}>
      ...
      <CardContent sx={{ height: "100%", p: 3, pb: 1,paddingTop: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
        {loading ? (
